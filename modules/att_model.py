...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, tag_ids, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, tag_ids, fc_feats, att_feats, p_att_feats, state, att_masks)
    ...
    def _sample_beam(self, tag_ids, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        logprobs, state = self.get_logprobs_state(it, tag_ids, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
        ...
        self.done_beams = self.beam_search(state, tag_ids, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, tag_ids, att_masks=None, update_opts={}):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(tag_ids, fc_feats, att_feats, att_masks, opt)
        if group_size > 1:
